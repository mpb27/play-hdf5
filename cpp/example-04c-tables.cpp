...
int main() {
    // Initialize a file
    h5pp::File file("exampledir/example-04c-tables.h5", h5pp::FilePermission::READONLY,0);

    // Register the compound type
    h5pp::hid::h5t H5_PARTICLE_TYPE = H5Tcreate(H5T_COMPOUND, sizeof(ParticleV0));
    H5Tinsert(H5_PARTICLE_TYPE, "x", HOFFSET(ParticleV0, x), H5T_NATIVE_DOUBLE);
    H5Tinsert(H5_PARTICLE_TYPE, "y", HOFFSET(ParticleV0, y), H5T_NATIVE_DOUBLE);
    H5Tinsert(H5_PARTICLE_TYPE, "z", HOFFSET(ParticleV0, z), H5T_NATIVE_DOUBLE);

    // Or read multiple records into a resizeable container. Start from index 0 and read 10 records.
    auto particles_read = file.readTableRecords<std::vector<ParticleV0>>("somegroup/particleTable", 0, 10);
    h5pp::print("Multiple record read:\n");
    for(auto &&p : particles_read) h5pp::print("x:{:.3f} y:{:.3f} z:{:.3f} t:{:.3f}\n",p.x,p.y,p.z);
