...
struct Int1 {
    int x;
    [[nodiscard]] std::string string() const { return h5pp::format("x: {}", x); }
};
...
void readExpanded(h5pp::File &file, const std::vector<Struct> &dset, std::string_view dsetName) {
    std::vector<Struct> dsetRead;
    file.readDataset(dsetRead, dsetName); // Read data.
    h5pp::print("Read  dataset [{}]:\n", dsetName);
    for(auto &c : dsetRead) h5pp::print("{}\n", c.string());
}
...
template<typename Struct>
void readReduced(h5pp::File &file, const std::vector<Struct> &dset, std::string_view dsetName) {
    std::vector<Struct> dsetRead;
    file.readDataset(dsetRead, dsetName); // Read data.
    h5pp::print("Read  dataset [{}]:\n", dsetName);
    for(auto &c : dsetRead) h5pp::print("{}\n", c.string());
...
int main() {
    ...
    std::vector<Int3> expandedData;
   // readExpanded(file, expandedData, "CoordinatesInTwoDimensions");

    std::vector<Int1> reducedData;
    readReduced(file, reducedData, "CoordinatesInTwoDimensions");
